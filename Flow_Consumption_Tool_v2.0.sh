...
LOGFILE=${LOGFILE:-/var/log/Flow_Consumption_Tool.log}

# 临时文件路径（用于状态持久化和锁）
LOCKFILE=${LOCKFILE:-/tmp/Flow_Consumption_Tool.lock}
MAINTOTAL_FILE=${MAINTOTAL_FILE:-/tmp/Flow_Consumption_Tool_main_total.bytes}
...
请选择操作（输入数字并回车）：
 ...
 4) 修改时间窗口并重启服务
 5) 监控实时网速（被动监控网口 RX，默认 5 秒）
 6) 启动主动测速（清华/腾讯/华为源 大文件下载，默认 10 秒）
 7) 服务状态
 8) 查看日志（实时追踪）
 9) 实时监控面板 (查看当前总速度与各线程状态)
 0) 退出
EOF
}
...
drain_stdin() {
  while read -t 0 -r -s _; do :; done
}

# 检测是否为 OpenWrt 环境
is_openwrt() {
  if [ -f /etc/openwrt_release ] || [ -f /etc/openwrt_version ] || ( [ -d /etc/init.d ] && command -v procd >/dev/null 2>&1 ); then
    return 0
  fi
  return 1
}
...
install_service_interactive() {
  ...
  echo "  2) ${START_TIME_2} - ${END_TIME_2}"
  echo "  3) ${START_TIME_3} - ${END_TIME_3}"
  
  # 提示选择主线程数 (0-16)，默认 0
  # 此处按照用户要求：默认有一个额外的保底线程，这里的输入是“额外”的（其实就是主线程 THREADS）
  read -p "请输入额外主线程数 (0-16, 本工具已含1个大文件线程，直接回车默认为 0): " threads_input
  threads_input=${threads_input:-0}
  if ! [[ "$threads_input" =~ ^[0-9]+$ ]] || [ "$threads_input" -gt 16 ]; then
    echo "输入无效，使用默认值 0"
    threads_input=0
  fi

  # 提示选择限速 (MB/s)，默认 10
  read -p "请输入限速 (MB/s, 默认 10): " speed_input
  speed_input=${speed_input:-10}
  ...
  if [ ! -d "$dest_dir" ]; then
    if ! run_as_root mkdir -p "$dest_dir" 2>/dev/null; then
      echo "错误：无法创建安装目录 $dest_dir。请手动创建或以 root 运行安装。" >&2
      return 2
    fi
  fi
...
EOF
    echo "已写入环境文件 $ENV_PATH"

    if is_openwrt; then
       # OpenWrt procd 适配
       INIT_SCRIPT="/etc/init.d/${SERVICE_NAME}"
       echo "检测到 OpenWrt 环境，正在创建 Procd init 脚本: $INIT_SCRIPT"
...
start_service() {
    procd_open_instance
    # 使用 set -a 确保 source 的变量被 export，从而传递给 exec 的脚本
    procd_set_param command /bin/sh -c "[ -f \"\$ENV_FILE\" ] && set -a && . \"\$ENV_FILE\" && set +a; exec \$PROG >> \$LOG_FILE 2>&1"
...
EOF
      ...
      run_as_root "$INIT_SCRIPT" enable
      run_as_root "$INIT_SCRIPT" start
      echo "OpenWrt 服务已安装并尝试启动(日志位于 /var/log/${SERVICE_NAME}.log)。"
...
refresh_config_interactive() {
  ...
  read -p "请输入限速 (MB/s, 直接回车保持不变): " speed_input

  if [ -f "$ENV_PATH" ]; then
    # 读取现有值作为默认回退
    current_threads=$(grep -E '^THREADS=' "$ENV_PATH" | tail -n1 | cut -d= -f2)
    current_limit=$(grep -E '^LIMIT_MBPS=' "$ENV_PATH" | tail -n1 | cut -d= -f2)
  fi

  threads_input=${threads_input:-$current_threads}
  speed_input=${speed_input:-$current_limit}

  if ! [[ "$threads_input" =~ ^[0-9]+$ ]] || [ "$threads_input" -gt 16 ]; then
    echo "输入无效，保持原线程数: ${current_threads:-0}"
    threads_input=${current_threads:-0}
  fi
  if ! echo "$speed_input" | grep -Eq '^[0-9]+(\.[0-9]+)?$'; then
    echo "输入无效，保持原限速: ${current_limit:-10}"
    speed_input=${current_limit:-10}
  fi
...
LIMIT_MBPS=${speed_input}
THREADS=${threads_input}
EXTRA_BACKUP_THREAD=${EXTRA_BACKUP_THREAD}
START_TIME_1=${START_TIME_1}
END_TIME_1=${END_TIME_1}
START_TIME_2=${START_TIME_2}
END_TIME_2=${END_TIME_2}
...
EOF
  ...
  if is_openwrt; then
    ...
    if [ -x "$INIT_SCRIPT" ]; then
      run_as_root "$INIT_SCRIPT" restart
      echo "OpenWrt 服务已重启以应用新配置。"
    else
      echo "警告：未找到 OpenWrt 服务脚本，建议先安装服务。"
    fi
  else
    run_as_root systemctl daemon-reload 2>/dev/null || true
    run_as_root systemctl restart "${SERVICE_NAME}.service" 2>/dev/null || true
    echo "Systemd 服务已重启以应用新配置。"
  fi
}

# 修改时间窗口并重启服务
modify_time_window_interactive() {
  if ! ensure_sudo; then
    return 1
  fi
  ...
  read -p "窗口1 结束(${END_TIME_1}): " e1
  read -p "窗口2 开始(${START_TIME_2}): " s2
  read -p "窗口2 结束(${END_TIME_2}): " e2
  read -p "窗口3 开始(${START_TIME_3}): " s3
  read -p "窗口3 结束(${END_TIME_3}): " e3
  ...
  # 简单格式校验
  for t in "$s1" "$e1" "$s2" "$e2" "$s3" "$e3"; do
    if ! echo "$t" | grep -Eq '^[0-9]{2}:[0-9]{2}$'; then
      echo "时间格式无效：$t (应为 HH:MM)"
      return 1
    fi
  done

  START_TIME_1="$s1"; END_TIME_1="$e1"
  START_TIME_2="$s2"; END_TIME_2="$e2"
  START_TIME_3="$s3"; END_TIME_3="$e3"
...
# 交互式测速：测量指定网口的出口带宽（主动下载）
test_iface_active() {
  local dur=${1:-10} # 默认测速时间
  local user_agent="Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"

  echo "正在寻找最优测速节点 (优先尝试 清华/腾讯/华为 镜像源)..."
  
  # 定义测速源列表
  # 策略：使用知名镜像站的 ISO 文件，比 API 生成文件更稳定且能跑满带宽
  local test_urls=(
    "https://mirrors.tuna.tsinghua.edu.cn/ubuntu-releases/24.04.3/ubuntu-24.04.3-desktop-amd64.iso"
    "https://mirrors.cloud.tencent.com/centos/7/updates/x86_64/Packages/thunderbird-115.12.1-1.el7.centos.x86_64.rpm"
    "https://mirrors.huaweicloud.com/ubuntu-releases/24.04.3/ubuntu-24.04.3-desktop-amd64.iso"
  ...
  for url in "${test_urls[@]}"; do
    # 快速探测 Header (2秒超时)
    if curl -I -L -A "$user_agent" --max-time 2 "$url" >/dev/null 2>&1; then
      target_url="$url"
      echo "已选用节点: $url"
      break
    fi
  ...
  if [ -z "$target_url" ]; then
    echo "错误：无法连接到任何测速节点，请检查网络或 DNS。"
    return 1
  fi
  ...
  if [ -z "$iface" ] || [ ! -r "$rx_file" ]; then
    echo "警告：无法获取网口流量统计信息，将仅尝试进行负载测试..."
  fi
  
  echo "开始主动测速，限时 ${dur} 秒..."
  echo "正在全速下载 (不写入磁盘)，请稍候..."
  echo "---------------------------------------------------"
  ...
  local start_ts=0
  if [ -r "$rx_file" ]; then
    start_bytes=$(cat "$rx_file")
    start_ts=$(date +%s.%N)
  fi

  # 启动后台下载任务 (静默模式，超时只要不报错炸脚本就行)
  curl -L -k -A "$user_agent" \
       --max-time "$dur" \
       -o /dev/null \
       -s \
       "$target_url" || true &
  ...
  # 计算结果
  local delta=$((end_bytes - start_bytes))
  local elapsed=$(awk "BEGIN {printf \"%.6f\", $end_ts - $start_ts}")
  
  # 排除时间过短的异常情况
  if awk "BEGIN {exit !($elapsed < 0.1)}"; then
     echo "测速异常：耗时过短。"
     return 1
  fi
  ...
  # delta是字节，elapsed是秒
  local speed_mb
  speed_mb=$(awk "BEGIN {printf \"%.2f\", ($delta / 1024 / 1024) / $elapsed}")
  
  local result_str=$(awk "BEGIN {
    speed = $speed_mb;
    if (speed >= 1024) {
      printf \"%.2f GB/s\", speed/1024
    } else {
      printf \"%.2f MB/s\", speed
    }
  }")
  
  local total_str=$(awk "BEGIN {
    bytes = $delta;
    if (bytes >= 1073741824) {
  ...
  echo "测速完成！"
  echo "监测网口    : $iface"
  echo "平均下载速度: ${result_str}"
  echo "总消耗流量  : ${total_str}"
}

# 交互式监控：被动监控网口流量（统计期间内 RX 增量）
test_iface_passive() {
  dur=${1:-5}
  echo "默认监控持续时间: ${dur}s"
  ...
  if [ ! -r "$rx_file" ]; then
    echo "无法读取 $rx_file，请检查权限或网口名 (iface=$iface)"
    return 1
  fi
  echo "监控网口: $iface 下行流量, 持续 ${dur}s..."
  ...
  delta=$((now - prev))
  elapsed=$(awk "BEGIN {printf \"%.2f\", $now_ts - $prev_ts}")
  
  speed_mb=$(awk "BEGIN {if ($elapsed>0) printf \"%.2f\", ($delta)/($elapsed*1024*1024); else print \"0.00\"}")
  
  result_str=$(awk "BEGIN {
    speed=$speed_mb;
    if (speed >= 1024) {
      printf \"%.2f GB/s\", speed/1024
    } else {
  ...
  total_str=$(awk "BEGIN {
    ...
    if (bytes >= 1000000000) {
      ...
      printf \"%.2f MB\", bytes/1000000
    } else {
      printf \"%d Bytes\", bytes
    }
  }")
  
  echo "$(date '+%F %T') iface=$iface 下行流量=${total_str} 耗时=${elapsed}s 平均速度=${result_str}"
}
...
status_service_interactive() {
  if ensure_sudo; then
    if is_openwrt; then
       echo "OpenWrt 环境下检查进程..."
       # OpenWrt procd 没有详细的 status 输出，用 ps 检查
       if pgrep -f "${SERVICE_NAME}.sh" >/dev/null; then
         echo "服务正在运行，PID: $(pgrep -f "${SERVICE_NAME}.sh" | tr '\n' ' ')"
       else
         echo "服务未运行。"
       fi
       echo "Init 脚本: /etc/init.d/${SERVICE_NAME}"
    else
...
view_logs_interactive() {
  if ensure_sudo; then
    echo "按 Enter 返回主菜单（日志追踪将停止）"
    drain_stdin
    if is_openwrt; then
       # OpenWrt: 优先尝试读取文件日志，无论 logread 是否存在
       local logfile="/var/log/${SERVICE_NAME}.log"
       if [ -f "$logfile" ]; then
         echo "正在追踪日志文件: $logfile"
         tail -f "$logfile" &
         log_pid=$!
         read -r _
         ...
         echo "未找到日志文件 $logfile 且系统无 logread 工具。"
         echo "请尝试先安装服务(选项1)以启用文件日志功能。"
       fi
    else
...
show_dashboard() {
  ...
  # 进入备用屏幕，避免污染原输出（优先 tput，否则使用 ANSI 1049）
  ...
  # 隐藏光标，退出时恢复并返回主屏幕
  printf "\033[?25l"
  trap 'printf "\033[?25h"; if command -v tput >/dev/null 2>&1; then tput rmcup; else printf "\033[?1049l"; fi; exit' INT TERM
  ...
  while true; do
    ...
    t1=$(date +%s%N)
    sleep 1
    r2=$(grep "$interface" /proc/net/dev 2>/dev/null | awk '{print $2}')
    t2=$(date +%s%N)
    ...
    delta_b=$(( r2-r1 ))
    [ "$delta_t" -eq 0 ] && delta_t=1000
    speed=$(awk "BEGIN {printf \"%.2f\", ($delta_b * 1000) / ($delta_t * 1000000)}")
    ...
    allowed=$(cat "$THREADS_FILE" 2>/dev/null || echo "")
    if ! [[ "$allowed" =~ ^[0-9]+$ ]]; then
      allowed=$THREADS
    fi
    if [ "$THREADS" -le 0 ]; then
      allowed=0
    elif [ "$allowed" -gt "$THREADS" ]; then
      allowed=$THREADS
    fi
    ...
    render_buf+=" 活跃主线程数 : ${allowed} / ${THREADS}"$'\n'
    render_buf+=" --------------------------------------------------------"$'\n'
    render_buf+=" -------------+------------------------------------------"$'\n'
    ...
    # 一次性输出整帧，避免闪烁；并清屏到末尾，清除残留字符
    printf "\033[H%s\033[J" "$render_buf"

    # 按 Enter 返回菜单（非阻塞）
    if read -t 0.1 -r -s key; then
      break
    fi
  done
}
...
if [ "$#" -eq 0 ] && [ -t 0 ]; then
  ...
  while true; do
    ...
    case "$choice" in
      ...
      3) refresh_config_interactive; continue;;
      4) modify_time_window_interactive; continue;;
      5) read -p "监控持续秒数(默认5): " t; t=${t:-5}; test_iface_passive "$t"; press_enter_to_return; continue;;
      6) read -p "测速持续秒数(默认10): " t; t=${t:-10}; test_iface_active "$t"; press_enter_to_return; continue;;
      7) status_service_interactive; press_enter_to_return; continue;;
      8) view_logs_interactive; press_enter_to_return; continue;;
      9) show_dashboard; continue;;
      0) echo "退出"; exit 0;;
      *) echo "无效选项";;
    esac
  done
...
STOP=0
# 增强信号处理：收到停止信号时，同时清理已知的后台子进程
cleanup_handler() {
  STOP=1
  # 遍历 pids 数组清理子进程 (数组在脚本末尾定义和填充)
  if [ -n "${pids[*]}" ]; then
     # Use kill to terminate children
...
download_one() {
  ...
  if [ "$cur_threads" -gt "$THREADS" ]; then cur_threads=$THREADS; fi
  ...
  if [ "$total_active" -lt 1 ]; then total_active=1; fi
    ...
    # 计算单线程限速 (bytes/s)
    # 直接在 awk 中计算： int(LIMIT_MBPS * 1000 * 1000 / total_active)
  local per_thread_limit_bytes
  per_thread_limit_bytes=$(awk -v limit="$LIMIT_MBPS" -v active="$total_active" 'BEGIN {
     if (active < 1) active = 1;
      val = (limit * 1000 * 1000) / active;
     printf "%.0f", val
  }' 2>/dev/null)
  
  # 兜底：如果 awk 失败或返回空，使用 Shell 整数运算 (不支持小数限速输入)
  if [ -z "$per_thread_limit_bytes" ] || ! [[ "$per_thread_limit_bytes" =~ ^[0-9]+$ ]]; then
      local safe_limit=${LIMIT_MBPS%.*}
      safe_limit=${safe_limit:-15}
        per_thread_limit_bytes=$(( safe_limit * 1000000 / total_active ))
  fi
  # ------------------------------------------------
  ...
  # 加 || true 防止 set -e (虽然已关闭) 或其他 shell 异常退出
  size_raw=$(curl -s --max-time 120 --limit-rate "${per_thread_limit_bytes}" -w "%{size_download}" -o /dev/null "$url" 2>/dev/null || true)
  rc=$?
  end_ts=$(get_time)
...
thread_worker() {
  ...
  while [ "$STOP" -eq 0 ]; do
    ...
    if [ "$id" -ge "$allowed_threads" ]; then
      update_mode_status "$t_name" "已暂停 (内存限制)"
      # 当前内存紧张，暂停此线程
      sleep 5
      continue
    fi
    ...
    while [ "$(date +%s)" -lt "$end_epoch" ] && [ "$STOP" -eq 0 ]; do
      ...
      while [ $attempt -le $RETRY_COUNT ] && [ "$(date +%s)" -lt "$end_epoch" ] && [ "$STOP" -eq 0 ]; do
        ...
        # 优化：移除进程替换
        output=$(download_one "$MAIN_URL" 2>/dev/null) || output="-1 0"
        read size dur <<< "$output"
  ...
  if [[ "$size" =~ ^[0-9]+$ ]] && [ "$size" -ge 0 ]; then
      ...
      while [ $attempt -le $RETRY_COUNT ] && [ "$(date +%s)" -lt "$end_epoch" ] && [ "$STOP" -eq 0 ]; do
        ...
        # read size dur < <(download_one "$SECONDARY_URL" 2>/dev/null)
        output=$(download_one "$SECONDARY_URL" 2>/dev/null) || output="-1 0"
        read size dur <<< "$output"
  ...
  if [[ "$size" =~ ^[0-9]+$ ]] && [ "$size" -ge 0 ]; then
      ...
      while [ $attempt -le $RETRY_COUNT ] && [ "$(date +%s)" -lt "$end_epoch" ] && [ "$STOP" -eq 0 ]; do
        ...
        # 优化：移除进程替换
        output=$(download_one "$url" 2>/dev/null) || output="-1 0"
        read size dur <<< "$output"
  ...
  if [[ "$size" =~ ^[0-9]+$ ]] && [ "$size" -ge 0 ]; then
      ...
      if [ "$sleep_time" -gt 0 ]; then
        update_mode_status "$t_name" "冷却中 (${sleep_time}s)"
        sleep "$sleep_time"
      fi
    done
...
monitor_stats() {
  ...
  while [ "$STOP" -eq 0 ]; do
    ...
    if [ "$now_hour_str" != "$last_hour_str" ]; then
      ...
      # 容错：BusyBox date 可能不支持 -d @timestamp
      period_end_str=$(date -d "@$now_sec" '+%H:%M:%S' 2>/dev/null || date '+%H:%M:%S')
...
monitor_memory() {
  ...
  while [ "$STOP" -eq 0 ]; do
    ...
    if [ -r /proc/meminfo ]; then
       # 从 /proc/meminfo 精确计算 (MemTotal - MemAvailable) / MemTotal * 100
       # 如果没有 MemAvailable (老内核)，用 MemFree + Buffers + Cached 近似
       mem_percent=$(awk '/MemTotal/{t=$2} /MemAvailable/{a=$2} END{if(t>0 && a>0) printf "%.0f", (t-a)/t*100; else print 0}' /proc/meminfo 2>/dev/null || echo 0)
    fi
    
    # 如果上面方法失败（比如 awk 返回0），尝试 free 命令兜底
    if [ "$mem_percent" -eq 0 ] && command -v free >/dev/null 2>&1; then
       mem_percent=$(free | awk '/Mem:/{if($2>0) printf("%.0f"), $3/$2 * 100; else print 0}' 2>/dev/null || echo 0)
    fi
...
echo "开始执行... $(date '+%F %T')"
# 打印调试信息到日志
echo "Debug: THREADS=$THREADS, EXTRA_BACKUP_THREAD=$EXTRA_BACKUP_THREAD, LIMIT_MBPS=$LIMIT_MBPS"
echo "脚本启动: 主线程(${THREADS}) + 大文件线程(${EXTRA_BACKUP_THREAD})，时间窗口为 ${START_TIME_1}-${END_TIME_1}, ${START_TIME_2}-${END_TIME_2}, ${START_TIME_3}-次日${END_TIME_3}，限速 ${LIMIT_MBPS}MB/s，每天循环往复"

# 初始化 PID 数组
pids=()

# 启动内存监控后台进程 (仅当有主线程需要管理时启动)
if [ "$THREADS" -gt 0 ]; then
  monitor_memory &
  pids+=($!)
fi
